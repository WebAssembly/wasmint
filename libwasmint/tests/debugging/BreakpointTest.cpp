...
public:
    virtual void reachedBreakpoint(const Breakpoint& breakpoint) {
...
int main() {
    ...
    const Instruction* breakpointInstruction = module->functions().front()->mainInstruction()->children().at(0);

    vm.useModule(*module, true);
    vm.startAtFunction(*module->functions().front());
    vm.addBreakpoint(breakpointInstruction, &handler);

    vm.stepUntilFinished(true);

    assert(breakpointInstruction == instruction);
    assert(!vm.gotTrap());
    assert(!vm.finished());
